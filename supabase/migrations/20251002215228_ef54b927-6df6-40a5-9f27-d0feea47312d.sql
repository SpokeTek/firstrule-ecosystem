-- Fix security issues - drop and recreate policies correctly

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Users can view own release distributions" ON release_distributions;
DROP POLICY IF EXISTS "Users can view own revenue reconciliation" ON revenue_reconciliation;
DROP POLICY IF EXISTS "Users can view own voice model analytics" ON voice_model_analytics;
DROP POLICY IF EXISTS "Service role can manage webhook events" ON openplay_webhook_events;

-- 1. Add RLS policies for release_distributions
...
-- 2. Add RLS policies for revenue_reconciliation
...
-- 3. Add RLS policies for voice_model_analytics
